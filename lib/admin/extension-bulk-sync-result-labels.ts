...
export function buildBulkSyncResultMessage(options: {
  action: string;
  reason?: string;
  message?: string;
  success?: boolean;
}): string {
  return formatBulkSyncResultMessage(options).message;
}
...
export function normalizeBulkSyncResultEntry<
  T extends { action: string; success?: boolean; reason?: string; message?: string },
>(entry: T): T {
...
export function countBulkSyncResultsByAction<
  T extends { product_id: number; reason?: string; action: string; success?: boolean },
>(results: T[]): {
  success: number;
  failed: number;
  unpublished: number;
  ...
  const normalized = results.map((entry) => normalizeBulkSyncResultEntry(entry));
  const deduped = dedupeBulkSyncResultsByProductId(normalized);
  const counts = {
    success: 0,
    failed: 0,
    unpublished: 0,
    ...
    uncertain: 0,
    skipped: 0,
    completed: deduped.length,
  };

  for (const entry of deduped) {
    if (entry.action === "updated" && entry.success !== false) {
      counts.success += 1;
      continue;
    }

    if (entry.action === "republished" && entry.success !== false) {
      counts.success += 1;
      counts.republished += 1;
      continue;
    }

    if (entry.action === "unpublished" && entry.success !== false) {
      counts.unpublished += 1;
      continue;
    }

    if (entry.action === "uncertain" && entry.success !== false) {
      counts.uncertain += 1;
      continue;
    }
